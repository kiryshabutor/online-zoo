...
document.addEventListener('click', (e)=>{
  ...
  else if( e.target.closest('.testimonials__swiper') == commentArea && commentArea !=null){
    if( window.innerWidth < 1000 ){
      comments.forEach( el => {
        if( el == e.target.closest('.comment')  ){
          ...
          popOver.querySelector('.comment__wrapper').append(commentBtn)
  ...
  else if( e.target != menuBlock && ( e.target.closest('.comment') != removeComment  || removeComment == null  || e.target.closest('.comment__btn') != null )  && body.classList.contains('lock')){
